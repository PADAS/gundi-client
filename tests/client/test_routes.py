...
@pytest.mark.asyncio
async def test_get_route_details(auth_token_response, route_details, gundi_client_v2):
    async with respx.mock(assert_all_called=False) as gundi_portal_mock:
        ...
        route = await gundi_client_v2.get_route_details(route_id=route_id)
        assert isinstance(route, Route)
        assert route == Route.parse_obj(route_details)


@pytest.mark.asyncio
async def test_get_routes_parses_results_envelope(
...
@pytest.mark.asyncio
async def test_get_routes_parses_bare_list(
    auth_token_response, route_details, gundi_client_v2
):
    async with respx.mock(assert_all_called=False) as mock:
        mock.post(gundi_client_v2.oauth_token_url).respond(
...
@pytest.mark.asyncio
async def test_get_routes_for_connection_filters_by_provider(
    auth_token_response, route_details, gundi_client_v2
):
...
@pytest.mark.asyncio
async def test_create_route_posts_with_data(
    auth_token_response, route_details, gundi_client_v2
):
    payload = {
...
@pytest.mark.asyncio
async def test_create_route_raises_gundi_api_error_on_400(
...
@pytest.mark.asyncio
async def test_update_route_patches_with_partial_data(
    auth_token_response, route_details, gundi_client_v2
):
    ...
    async with respx.mock(assert_all_called=False) as mock:
        ...
        patch_route = mock.patch(
            f"{gundi_client_v2.routes_endpoint}/{route_id}/"
        ).respond(status_code=httpx.codes.OK, json=route_details)
...
@pytest.mark.asyncio
async def test_update_route_raises_gundi_api_error_on_400(
    auth_token_response, gundi_client_v2
):
    route_id = "bogus-id"
    async with respx.mock(assert_all_called=False) as mock:
...
@pytest.mark.asyncio
async def test_delete_route_issues_delete_and_returns_none(
    auth_token_response, route_details, gundi_client_v2
):
    route_id = route_details["id"]
    async with respx.mock(assert_all_called=False) as mock:
