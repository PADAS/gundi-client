...
4. `force_refresh_token=True` (the auth-realm redirect in `_get`, `_post`,
   ...
   stops trying that refresh token; any other status, whatever its body says,
   leaves everything in place, and a transport failure on the refresh grant
   (`AuthenticationError.transport`, set by `auth._post_token`) is raised
   without attempting the full grant (amended 2026-09-08,
